	...
	return new Promise((resolve, reject) => {
		 ...
		 axios({
			  ...
			  axios({
				   ...
				   resolve({
						status: true,
						message: "Created By Mr Enigmatic",
						result: result
				   })
			  }).catch(reject)
		 }).catch(reject)
...
exports.buffergif = async (image) => {
	...
	const filename = `${Math.random().toString(36)}`
			await fs.writeFileSync(`./VeroMedia/trash/${filename}.gif`, image)
					 child_process.exec(
								`ffmpeg -i ./VeroMedia/trash/${filename}.gif -movflags faststart -pix_fmt yuv420p -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" ./VeroMedia/trash/${filename}.mp4`
										) 
  await sleepy(4000)
  
	var buffer5  =  await  fs.readFileSync(`./VeroMedia/trash/${filename}.mp4`)
	Promise.all([unlink(`./VeroMedia/video/${filename}.mp4`), unlink(`./VeroMedia/gif/${filename}.gif`)])
	return buffer5
				   }