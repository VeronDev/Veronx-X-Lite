...
exports.time = (numer, options = {}) => {
    let format = options.format ? options.format : "HH:mm"
    let time = options?.timeZone ? moment(numer).tz(timeZone).format(format) : moment(numer).format(format)
    return `${time}`
}
...
exports.getSizeMedia = (path) => {
    return new Promise((resolve, reject) => {
        if (/http/.test(path)) {
            ...
            reject('Error: Unknown media type')
        }
    })
}
...
exports.smsg = (XeonBotInc, m, store) => {
    ...
    if (m.message) {
        ...
        // Fixed: Added proper null checks to prevent "cannot read properties of undefined" errors
        ...
        if (m.quoted) {
            ...
            // Fixed: Added null checks for quoted message properties
    ...
    // Fixed: Added null check for m.msg
    if (m.msg && m.msg.url) m.download = () => XeonBotInc.downloadMediaMessage(m.msg)
...
exports.reSize = (buffer, width, height) => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            var ab = await baper.resize(width, height).getBufferAsync(Jimp.MIME_JPEG)
            resolve(ab)
        } catch (error) {
            reject(error)
        }
